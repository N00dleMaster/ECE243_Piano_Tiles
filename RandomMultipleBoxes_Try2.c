...
int main(void)
{
    ...
    int directions[4] = {1, 1, 1, 1}; // Directions of movement for each square (1 for down, -1 for up)
    ...
    while(1) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (frame_counter >= delays[i]) {
                ...
                // Update y position and direction if needed
                if(y_positions[i] == 0) {
                    directions[i] = 1;
                } else if (y_positions[i] + height >= 239) {
                    directions[i] = -1;
                }

                y_positions[i] += directions[i]; // Update y position
